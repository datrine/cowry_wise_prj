import sqlite3
from app.repository.borrow_list import (save_borrow_list_item,get_borrow_list_item_by_id,get_borrow_list)
...
def test_save_borrow_list_item_no_return_date_id_error(app_context,existing_user,existing_book):
    return_date=None
    with app_context:
        try:
...
def existing_borrow_list_item_by_id(app_context,existing_borrow_list_item:dict):
    with app_context:
        book= dict( get_borrow_list_item_by_id(id=existing_borrow_list_item.get("id")) )
        assert book is not None
